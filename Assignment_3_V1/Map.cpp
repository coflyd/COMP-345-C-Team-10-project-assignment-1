...
bool Map::areAdjacent(int countryId1, int countryId2)  {
	auto it = neighboursEdge.find(countryId1);
	if (it == neighboursEdge.end())
		return false;
	for (int neighbor : it->second) {
		if (neighbor == countryId2)
			return true;
	}
	return false;
}
...
bool Map::isAdjacentToAnyOwnedCountry(Country *country,
		 const std::vector<Country*> &ownedCountries)  {
	for(auto c:ownedCountries){
		if ( ! Map::areAdjacent(country->id, c->id)){
			return (false);
		}
	}
	return (true);
...
Country* Map::findStrongestAdjacentCountry(Country *target,
		const std::vector<Country*> &ownedCountries)  {
	Country *strongest = nullptr;
	int maxArmies = 0;

	for (Country *owned : ownedCountries) {
		if (Map::areAdjacent(target->id, owned->id )) {
			if (owned->armies > maxArmies) {
				maxArmies = owned->armies;
				strongest = owned;
			}
		}
	}
