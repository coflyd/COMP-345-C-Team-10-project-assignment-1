...
public:
	bool areAdjacent(int countryId1, int countryId2);
	...
	static bool isThisGraphConnected(const std::map<int, Country*> &nodes,
	...
	bool isAdjacentToAnyOwnedCountry(Country *country,
			const std::vector<Country*> &ownedCountries);
	Country* findStrongestAdjacentCountry(Country *target,
			const std::vector<Country*> &ownedCountries);
