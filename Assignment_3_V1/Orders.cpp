...
// Default
Order::Order() :
		description(new std::string("")), effect(new std::string("")), executed(
				new bool(false)) {
}

// With Descriptions
Order::Order(const std::string &desc) :
		description(new std::string(desc)), effect(new std::string("")), executed(
				new bool(false)) {
}

// Copy constructor with deep copy pointer members
Order::Order(const Order &other) :
		description(new std::string(*other.description)), effect(
				new std::string(*other.effect)), executed(
				new bool(*other.executed)) {
}
...
Deploy::Deploy(const std::string &targetName, int a) :
		Order("Deploy to " + targetName + " (" + std::to_string(a) + ")"), issuingPlayer(
				nullptr), targetCountry(nullptr), armies(new int(a)) {
}

Deploy::Deploy(Player *player, Country *target, int a) :
		Order(
				"Deploy to " + (target ? target->name : "?") + " ("
						+ std::to_string(a) + ")"), issuingPlayer(player), targetCountry(
				target), armies(new int(a)) {
}

// Copy constructor with deep copy pointer members
// issuingPlayer and targetCountry are not owned so they are shallow copied
Deploy::Deploy(const Deploy &other) :
		Order(other), issuingPlayer(other.issuingPlayer), targetCountry(
				other.targetCountry), armies(new int(*other.armies)) {
}
...
bool Deploy::validate() const {
	if (armies == nullptr || *armies <= 0)
		return false;
	if (issuingPlayer == nullptr || targetCountry == nullptr)
		return true;
	if (targetCountry->playerName == nullptr)
		return false;
	if (*targetCountry->playerName != issuingPlayer->getName())
		return false;
	if (issuingPlayer->getReinforcements() < *armies)
		return false;
	return true;
}
...
void Deploy::execute() {
	...
	if (issuingPlayer != nullptr && targetCountry != nullptr) {
		...
		setEffect(
				"Deployed " + std::to_string(*armies) + " armies to "
						+ targetCountry->name + ". Territory now has "
						+ std::to_string(targetCountry->armies) + " armies.");
	} else {
		setEffect(
				"Deployed " + std::to_string(*armies)
						+ " armies (convenience mode).");
	}
}
...
Advance::Advance(const std::string &from, const std::string &to, int a) :
		Order("Advance " + from + " -> " + to + " (" + std::to_string(a) + ")"), issuingPlayer(
				nullptr), fromCountry(nullptr), toCountry(nullptr), map(
				nullptr), deck(nullptr), armies(new int(a)) {
}

Advance::Advance(Player *player, Country *from, Country *to, Map *m, Deck *d,
		int a) :
		Order(
				"Advance " + (from ? from->name : "?") + " -> "
						+ (to ? to->name : "?") + " (" + std::to_string(a)
						+ ")"), issuingPlayer(player), fromCountry(from), toCountry(
				to), map(m), deck(d), armies(new int(a)) {
}

// Copy constructor with deep copy pointer members
// Game object pointers are not owned so they are shallow copied
Advance::Advance(const Advance &other) :
		Order(other), issuingPlayer(other.issuingPlayer), fromCountry(
				other.fromCountry), toCountry(other.toCountry), map(other.map), deck(
				other.deck), armies(new int(*other.armies)) {
}
...
bool Advance::validate() const {
	if (armies == nullptr || *armies <= 0)
		return false;
	if (issuingPlayer == nullptr || fromCountry == nullptr
			|| toCountry == nullptr)
		return true;

	if (fromCountry->playerName == nullptr)
		return false;
	if (*fromCountry->playerName != issuingPlayer->getName())
		return false;
	if (toCountry->playerName != nullptr
			&& *toCountry->playerName != issuingPlayer->getName()) {
		if (issuingPlayer->isTruceWith(*toCountry->playerName))
			return false;
	}
...
void Advance::execute() {
	...
	if (toCountry->playerName != nullptr
		...
		setEffect(
				"Moved " + std::to_string(moved) + " armies from "
						+ fromCountry->name + " to " + toCountry->name
						+ " (friendly transfer).");
		return;
	}
	...
	if (survivingDefenders == 0) {
		...
		std::string previousOwner =
				toCountry->playerName ? *toCountry->playerName : "Neutral";
		...
		if (!issuingPlayer->hasConqueredThisTurn() && deck != nullptr) {
			...
			if (reward != nullptr)
				issuingPlayer->getHand()->addCard(reward);
		}
		issuingPlayer->setConqueredThisTurn(true);
		setEffect(
				"Battle at " + toCountry->name + ": "
						+ std::to_string(survivingAttackers)
						+ " attackers survived, " + "all defenders eliminated. "
						+ issuingPlayer->getName() + " captured "
						+ toCountry->name + " from " + previousOwner + ".");
	} else {
		...
		setEffect(
				"Battle at " + toCountry->name + ": "
						...
						+ " defenders survived, " + "attack repelled. "
						+ std::to_string(survivingAttackers)
						+ " attackers returned to " + fromCountry->name + ".");
	}
}
...
Bomb::Bomb(const std::string &targetName) :
		Order("Bomb " + targetName), issuingPlayer(nullptr), targetCountry(
				nullptr), map(nullptr) {
}

Bomb::Bomb(Player *player, Country *target, Map *m) :
		Order("Bomb " + (target ? target->name : "?")), issuingPlayer(player), targetCountry(
				target), map(m) {
}
...
Bomb::Bomb(const Bomb &other) :
		Order(other), issuingPlayer(other.issuingPlayer), targetCountry(
				other.targetCountry), map(other.map) {
}
...
bool Bomb::validate() const {
	if (issuingPlayer == nullptr || targetCountry == nullptr || map == nullptr)
		return true;
	if (targetCountry->playerName != nullptr
			&& *targetCountry->playerName == issuingPlayer->getName())
		return false;
	if (targetCountry->playerName != nullptr
			&& issuingPlayer->isTruceWith(*targetCountry->playerName))
		return false;
	for (Country *owned : issuingPlayer->toDefend()) {
		if (map->areAdjacent(owned->id, targetCountry->id))
			return true;
	}
	return false;
}
...
void Bomb::execute() {
	...
	if (targetCountry != nullptr) {
		...
		setEffect(
				"Bombed " + targetCountry->name + ". Removed "
						+ std::to_string(removed) + " armies. "
						+ std::to_string(targetCountry->armies)
						+ " armies remain.");
	} else {
...
Blockade::Blockade(const std::string &targetName) :
		Order("Blockade " + targetName), issuingPlayer(nullptr), targetCountry(
				nullptr), neutralPlayer(nullptr) {
}

Blockade::Blockade(Player *player, Country *target, Player *neutral) :
		Order("Blockade " + (target ? target->name : "?")), issuingPlayer(
				player), targetCountry(target), neutralPlayer(neutral) {
}
...
Blockade::Blockade(const Blockade &other) :
		Order(other), issuingPlayer(other.issuingPlayer), targetCountry(
				other.targetCountry), neutralPlayer(other.neutralPlayer) {
}
...
void Blockade::execute() {
	if (!validate()) {
		setEffect(
				"Invalid Blockade: target does not belong to issuing player.");
		return;
	}
	if (targetCountry != nullptr) {
		...
		if (neutralPlayer != nullptr) {
			...
			targetCountry->playerName = new std::string(
					neutralPlayer->getName());
		} else {
			delete targetCountry->playerName;
			targetCountry->playerName = new std::string("Neutral");
		}
		setEffect(
				"Blockaded " + targetCountry->name + ". Armies doubled to "
						+ std::to_string(targetCountry->armies)
...
Airlift::Airlift(const std::string &from, const std::string &to, int a) :
		Order("Airlift " + from + " -> " + to + " (" + std::to_string(a) + ")"), issuingPlayer(
				nullptr), fromCountry(nullptr), toCountry(nullptr), armies(
				new int(a)) {
}

Airlift::Airlift(Player *player, Country *from, Country *to, int a) :
		Order(
				"Airlift " + (from ? from->name : "?") + " -> "
						+ (to ? to->name : "?") + " (" + std::to_string(a)
						+ ")"), issuingPlayer(player), fromCountry(from), toCountry(
				to), armies(new int(a)) {
}
...
Airlift::Airlift(const Airlift &other) :
		Order(other), issuingPlayer(other.issuingPlayer), fromCountry(
				other.fromCountry), toCountry(other.toCountry), armies(
				new int(*other.armies)) {
}
...
void Airlift::execute() {
	if (!validate()) {
		setEffect(
				"Invalid Airlift: source or target does not belong to issuing player.");
		return;
	}
	if (fromCountry != nullptr && toCountry != nullptr) {
		...
		toCountry->armies += moved;
		setEffect(
				"Airlifted " + std::to_string(moved) + " armies from "
						+ fromCountry->name + " to " + toCountry->name + ".");
	} else {
		setEffect(
				"Airlifted " + std::to_string(*armies)
						+ " armies (convenience mode).");
	}
}
...
Negotiate::Negotiate(const std::string &playerAName,
		const std::string &playerBName) :
		Order("Negotiate " + playerAName + " <-> " + playerBName), issuingPlayer(
				nullptr), targetPlayer(nullptr) {
}

Negotiate::Negotiate(Player *issuer, Player *target) :
		Order(
				"Negotiate " + (issuer ? issuer->getName() : "?") + " <-> "
						+ (target ? target->getName() : "?")), issuingPlayer(
				issuer), targetPlayer(target) {
}

// Copy constructor since player pointers are not owned so they are shallow copied
Negotiate::Negotiate(const Negotiate &other) :
		Order(other), issuingPlayer(other.issuingPlayer), targetPlayer(
				other.targetPlayer) {
}
...
void Negotiate::execute() {
	...
	if (issuingPlayer != nullptr && targetPlayer != nullptr) {
		...
		setEffect(
				"Truce established between " + issuingPlayer->getName()
